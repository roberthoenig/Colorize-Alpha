...
You may feel that manually running the `zulip-bot-server` command is a little cumbersome
and brittle: It needs a separate terminal to monitor and has to be restarted on failure.
For such cases, `supervisord` gives you a helping hand: Once you've dealt with a few configuration
settings, it automatically runs the botserver for you, telling you the server's current status,
restarting if needed, and keeping track of all logged information.

Running the Zulip Botserver with supervisord is straightforward.
...
1.  Configure `supervisord`:
    * If you have freshly installed supervisord:
      Download the <a href="https://raw.githubusercontent.com/Supervisor/supervisor/master/supervisor/skel/sample.conf" download>configuration file</a>
      and store it in `/etc/supervisor/conf.d/zulip-botserver.conf`. 
      Open the file with your favorite text editor and edit the `<>` sections accordingly.

    * If you already have a working supervisord instance:
...
2. Update `supervisord` to read the [new] configuration file:
   ```
